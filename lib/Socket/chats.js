...
const makeChatsSocket = (config) => {
        ...
        await authState.keys.transaction(async () => {
            ...
            while (collectionsToHandle.size) {
                ...
                for (const key in decoded) {
                    ...
                    try {
                        if (snapshot) {
                            const { state: newState, mutations } = await (0, Utils_1.decodeSyncdSnapshot)(name, snapshot, getAppStateSyncKey, initialVersionMap[name]);
                            states[name] = newState;
                            logger.info({ mutations: logger.level === 'trace' ? mutations : undefined }, `restored state of ${name} from snapshot to v${newState.version} with ${mutations.length} mutations`);
                            await authState.keys.set({ 'app-state-sync-version': { [name]: newState } });
                            appStateChunk.totalMutations.push(...mutations);
                        }
                        // only process if there are syncd patches
                        if (patches.length) {
                            const { newMutations, state: newState } = await (0, Utils_1.decodePatches)(name, patches, states[name], getAppStateSyncKey, initialVersionMap[name]);
                            await authState.keys.set({ 'app-state-sync-version': { [name]: newState } });
    ...
    const processSyncActionsLocal = (actions) => {
        const events = (0, Utils_1.processSyncActions)(actions, authState.creds.me, logger);
    ...
    const appPatch = async (patchCreate) => {
        ...
        await mutationMutex.mutex(async () => {
            ...
            initial = initial || (0, Utils_1.newLTHashState)();
